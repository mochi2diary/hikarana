...
- 画像は `img/` に置き，`![](ファイル名)` で `img/` からの相対パスを指定します。`img/0001/犬.png` のようにサブディレクトリで整理してもよく，その場合は `![](0001/犬.png)` と書きます。絶対パスや `..` は使えません。
- 画像は PDF 埋め込み時に 30mm×600dpi 相当(長辺 709px)の JPEG(品質 90)へ縮小されます。元画像は変更せず，変換結果は `tmp/imgcache/` に `img/` と同じディレクトリ構成でキャッシュされ，元画像が更新されると再変換されます。`tmp/` はいつ削除しても次回実行時に作り直されます。元画像は PNG でも JPEG でも構いません。元が既に長辺 709px 以下の JPEG なら再エンコードせずそのままコピーします(二重の不可逆圧縮を避けるため)。`--img-dpi` `--img-quality` で調整，`--img-converter vips|pillow|none` で変換器を固定，`--no-img-convert` で imgcache を作らず元画像のまま埋め込みます。
...
title: "ひらカナマスター"
number: "0001"
desc: "え と あうように わくのなかに じをかいてください。"
---
