...
- 画像は PDF 埋め込み時に 30mm×600dpi 相当(長辺 709px)の JPEG(品質 90)へ縮小されます。元画像は変更せず，変換結果は `tmp/imgcache/` に `img/` と同じディレクトリ構成でキャッシュされ，元画像が更新されると再変換されます。`tmp/` はいつ削除しても次回実行時に作り直されます。`--img-dpi` `--img-quality` で調整，`--img-converter vips|pillow|none` で変換器を固定，`--no-img-convert` で元画像のまま埋め込みます。
